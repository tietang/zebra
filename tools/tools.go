...
import (
	...
	kini "github.com/tietang/props/ini"
...
func FileToConsulKeyValue(file string, address, root string) {
	...
	if strings.Contains(ext, "prop") {
		...
		conf = kini.NewIniFileConfigSource(file)
	}
...
func FileToZookeeperKeyValue(file string, urls, root string) {
	...
	conn, ch, err := zookeeper.Connect([]string{urls}, 2*time.Second)
	if err != nil {
		panic(err)
	}
	for {
	...
	for _, key := range keys {
		...
		if !zk.ZkExits(conn, keyPath) {
			_, err := zk.ZkCreateString(conn, keyPath, value)
			if err == nil {
				//log.Println(path)
			}
			//fmt.Println(v)
...
func IniFileToZookeeperProperties(file, urls, root string) {
	...
	for _, section := range sections {
		...
		fmt.Println(val)
		if !zk.ZkExits(conn, keyPath) {
			_, err := zk.ZkCreateString(conn, keyPath, val)
			if err == nil {
				//log.Println(path)
			}
			//fmt.Println(v)
