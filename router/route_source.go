...
func GetGlobalRouterSources() []RouterSource {
	return globalRouterSources
}
...
func (p *KeyValueRouteSource) add(route *Route, isInTimeSync bool) {
	...
	for i, r := range p.routes {
		if r.Id == route.Id {
			hasExists = true
			p.routes[i] = route
			log.WithField("host", route).Debug("update host: ")
		}
		...
		if route.IsForceUpdate && route.ServiceId == r.ServiceId {
			hasExists = true
			p.routes[i] = route
			log.WithField("host", route).Debug("force update host: ")
		}
	}
	if !hasExists {
		p.routes = append(p.routes, route)
		log.WithField("host", route).Debug("add host: ")
	}
