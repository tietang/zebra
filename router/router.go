...
func (r *Router) AddRoute(route Route) {
	...
	for i, rt := range r.Routes {
		...
		if rt.Id == route.Id {
			log.Debug("host updated: ", i, ": ", rt.Id, " = ", route.Id)
			//rts := append(r.Routes[:i], host)
	...
	if len(r.Routes) == 0 || !hasExists {
		log.Debug("host append: ", route.Id)
		r.Routes = append(r.Routes, route)
	}

	//fmt.Println(len(r.Routes), host.ServiceId, host.Source, host.Target)
}
